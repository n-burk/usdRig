...
def _guardsQtBoundary(handler):
    """Stop a Python exception escaping into Qt's virtual dispatch.

    These handlers override C++ virtuals, so an exception raised inside one has
    to be carried back out through shiboken. That path is not robust in this
    PySide build: a mouse release whose handler raised took the process down
    with SIGSEGV inside PepException_GetArgs, reading 0xffffffffffffffff, with
    no traceback -- so the Python bug that started it was invisible, which is
    the worst part.

    event()'s own `except RuntimeError` cannot cover this. It wraps
    super().event(), and super().event() is where the boundary crash happens,
    so it only ever sees the failures shiboken survived long enough to report.
    Catching here, before the exception is handed back to C++, is what keeps
    the process alive AND produces a traceback to act on.

    The event is dropped, which for a mouse or key handler means one lost
    interaction rather than a lost session.
    """
    ...
    def guarded(self, event):
        try:
            ...
            Tf.Warn("%s.%s raised; the event was dropped. This is a bug -- "
                    "please report the traceback:\n%s"
...
def _nodeIdPrimPath(nodeId):
    """The prim-path half of a node id.

    Most node ids ARE prim paths. Blueprint / Container I/O nodes are
    synthesized rather than prim-backed and carry their pin in a ``#`` suffix
    (``/Bp#input:foo:bar``); the part before the ``#`` is still the prim path
    they hang off, which is what makes them follow that prim when it moves.
    """
    return nodeId.split("#", 1)[0]


def _isPathRelated(pathA, pathB):
    """Whether one path is the other, or contains the other.

    "Contains" spans both separators USD uses: ``/`` for a child prim and ``.``
    for a property, so ``/World/Cube`` is related to ``/World/Cube/Leg`` and to
    ``/World/Cube.inputs:x`` alike -- a resync can name either, and both mean
    "the node at /World/Cube is stale".

    The boundary is the point. Plain ``startswith`` reads ``/World/Cube`` as an
    ancestor of ``/World/CubeTwo``, so editing one prim invalidates unrelated
    siblings whose names merely share a prefix.
    """
    if pathA == pathB:
        return True
    longer, shorter = (pathA, pathB) if len(pathA) > len(pathB) else (pathB, pathA)
    if not longer.startswith(shorter):
        return False
    # The pseudo-root is the one path that already ends in its own separator.
    if shorter.endswith("/"):
        return True
...
def _make_rename_undo(gv, parent_path, from_name, to_name):
    """Return a callable that renames a sibling of *parent_path*.

    Named by (parent, name) rather than by a captured prim, because the prim
    the caller held is invalidated by the rename that this reverses -- the
    path it was fetched from no longer resolves.
    """

    def _fn(_parent=parent_path, _from=from_name, _to=to_name):
        from .primRename import RENAMED, rename_prim

        stage = gv.nodeGraph.getStage()
        if not stage:
            return
        oldPath = f"{_parent}/{_from}".replace("//", "/")
        prim = stage.GetPrimAtPath(oldPath)
        if not prim or not prim.IsValid():
            Tf.Warn("Rename undo: no prim named %s under %s" % (_from, _parent))
            return
        status, newPath, message = rename_prim(stage, prim, _to)
        if status != RENAMED:
            Tf.Warn("Rename undo failed (%s): %s" % (status, message))
            return
...
def _make_api_schema_undo(gv, identifier, applied, apply):
    """Return a callable that applies or removes an API schema.

    The instance names chosen when the schema was first applied are captured
    here so that redo reuses them instead of generating new ones.

    Args:
        gv: GraphView reference (prevents closure over self).
        identifier: USD schema identifier.
        applied: List of (primPath, instanceName) pairs; instanceName is ""
            for single-apply schemas.
        apply: True to apply the schema on redo, False to remove it on undo.
    """

    def _fn(_identifier=identifier, _applied=list(applied), _apply=apply):
        from .apiSchemas import apply_named, remove_from_prim

        stage = gv.nodeGraph.getStage()
        if not stage:
            return
        for path_str, instance_name in _applied:
            prim = stage.GetPrimAtPath(path_str)
            if not prim or not prim.IsValid():
                continue
            if _apply:
                apply_named(prim, _identifier, instance_name)
            else:
                remove_from_prim(prim, _identifier, instance_name)
        gv._reloadCurrentGraph()
...
class GraphView(QGLWidget):
    ...
    def __init__(self, parent=None, settingsObject=None):
        ...
        # Inline prim rename. Non-empty _renamingNodeId is the whole "a rename
        # is in progress" flag: it gates key routing, painting, and what a
        # mouse press means.
    ...
    def _updateNodeCreationHotbox(self):
        ...
        The applied API schemas registered in this build are appended after the
        libraries. They are not node types - picking one applies it to the
        selected prims rather than creating a prim.
        """
        ...
        node_items.extend(self._apiSchemaHotboxItems())

        # Configure the hotbox with the collected node items
        self.nodeCreationHotbox.configure_from_libraries(node_items)

    @staticmethod
    def _apiSchemaHotboxItems():
        """Build hotbox entries for every applied API schema in this build.

        Each entry is suffixed "*api" and grouped under the plugin that
        defines it, so the list reads as schemas to apply rather than prims to
        create. The marker doubles as a filter term: the list filters on a
        plain substring of the display name, and no prim type contains "*", so
        typing it narrows the hotbox to schemas alone. It stays out of the
        "identifier" field, which is what ApplyAPI receives.

        get_api_schemas() returns them sorted by plugin, which the list widget
        requires: it emits a group header whenever the group changes, so a
        non-contiguous group would repeat its header.
        """
        from .apiSchemas import API_SCHEMA_KIND, get_api_schemas

        return [
            {
                "name": f"{schema.identifier} *api",
                "family": f"API Schemas: {schema.plugin}",
                "identifier": schema.identifier,
                "library": None,
                "kind": API_SCHEMA_KIND,
            }
            for schema in get_api_schemas()
        ]
    ...
    def _graphScopeParentPath(self):
        """The prim whose children this graph shows.

        NodeGraphStage walks the pseudo-root's children; the blueprint and
        container graphs walk the children of the prim they were opened on.
        Reconciling against the stage has to use the same rule the loaders
        use, or a resync deep in the hierarchy would add a node this view was
        never meant to show.
        """
        from .nodeGraphBlueprint import NodeGraphBlueprint
        from .nodeGraphBpContainer import NodeGraphBpContainer

        if isinstance(self.nodeGraph, (NodeGraphBlueprint, NodeGraphBpContainer)):
            return str(self._currentPrimPath) if self._currentPrimPath else None
        return "/"

    def _validateNodesAgainstStage(self):
        """Drop every node whose prim is no longer on the stage.

        As far as USD notices are concerned a rename is a delete plus an add:
        the old path resyncs with nothing behind it any more. The resync loop
        above only invalidates caches for paths still present in self.nodes,
        so without this the old node keeps drawing from its cached name while
        everything that goes through the stage — selection, framing,
        connecting — silently does nothing, because the prim it names is gone.
        That is the "node stuck under the old name that will not select".

        Deactivated prims count as gone: the loaders skip inactive children,
        so keeping their nodes would leave the view disagreeing with what a
        reload would produce.

        Returns:
            list[str]: node ids removed.
        """
        stale = []
        for nodeId, node in list(self.nodes.items()):
            # Blueprint I/O nodes are synthesized, not backed by a prim.
            if "#" in nodeId:
                continue
            try:
        ...
        for nodeId in stale:
            ...
            self._marqueeBaseSelectedNodes.discard(nodeId)
            self._marqueePrevSelectedNodes.discard(nodeId)
            # _rebuildLinks rebuilds the whole index, but that is deferred to
            # the next paint; drop it now so a click landing first cannot hit
        ...
        if stale:
            Tf.Status("Graph validation dropped %d node(s) whose prims are "
                      "gone: %s" % (len(stale), ", ".join(sorted(stale))))
        return stale

    def _adoptPrimsNewToScope(self, resyncedPaths):
        """Add nodes for prims that appeared in this graph's scope.

        The other half of a rename: the new path resyncs with a real prim
        behind it and nothing in the graph pointing at it, so without this a
        renamed node disappears from the editor until a manual reload.

        Only paths in this notice are considered, and only direct children of
        the scope this graph loads. Note this can re-add a node the user hid
        with 'D' (which removes from the view without touching USD) if that
        same prim is later edited — the view follows the stage, which is the
        contract being asked for here.

        Returns:
            list[str]: node ids added.
        """
        stage = self.nodeGraph.getStage()
        scope = self._graphScopeParentPath()
        if not stage or scope is None:
            return []

        added = []
        for pathStr in sorted(resyncedPaths):
            if "." in pathStr or pathStr in self.nodes:
                continue
            prim = stage.GetPrimAtPath(pathStr)
            if not prim or not prim.IsValid() or not prim.IsActive():
                continue
            if str(prim.GetPath().GetParentPath()) != scope:
                continue
            try:
                nodeId = self._addNewNodeFast(prim, stage)
            except Exception as error:
                Tf.Warn("Graph validation could not add %s: %s" % (pathStr, error))
                continue
            if nodeId:
                added.append(nodeId)
        return added
    ...
    def _remapMovedNodes(self, movedPaths):
        """Carry nodes across a namespace move instead of dropping them.

        A node id IS a prim path, so a rename or reparent changes the identity
        of every node at or under the moved prim. The alternative -- rebuilding
        the graph from a loader -- cannot work here, because graph membership
        is not derivable from the stage: 'A' (``addNodesFromPrimTreeSelection``)
        puts prims at ANY depth on the canvas and 'D' takes them off again
        without touching USD, so the set of nodes on screen is the user's, not
        the loader's. Re-keying is the only operation that preserves it.

        Everything keyed by the old id moves with it: the node dict, the
        NodeModel's own identity and its expired ``Usd.Prim``, the selection and
        marquee sets, the spatial index, and the scattered cursor / drag / hover
        ids. Links are NOT re-keyed -- ``_rebuildLinks`` re-derives every one of
        them from USD, which is also what picks up the relationship targets and
        connections USD retargeted as part of the move.

        Idempotent: paths with no node behind them are skipped, so it is safe to
        call from the authoring site as well as from the notice, whichever gets
        there first.

        Args:
            movedPaths: ``{oldPrimPath: newPrimPath}`` as strings.

        Returns:
            dict[str, str]: the ``{oldNodeId: newNodeId}`` actually applied.
        """
        if not movedPaths:
            return {}
        ...
        # A move of /A implies a move of everything under it, but USD reports
        # only the prim that actually moved -- descendants are not enumerated in
        # the notice. Walk the graph instead and rewrite every id that sits at
        # or under a moved path.
        remap = {}
        for nodeId in self.nodes:
            primPath = _nodeIdPrimPath(nodeId)
            for oldPath, newPath in movedPaths.items():
                if not _isPathRelated(primPath, oldPath) or len(primPath) < len(
                    oldPath
                ):
                    continue
                newId = newPath + nodeId[len(oldPath):]
                if newId != nodeId:
                    remap[nodeId] = newId
                break
        ...
        for oldId, newId in remap.items():
            node = self.nodes[oldId]
            node.id = newId
            # Synthesized Blueprint I/O nodes are not prim-backed: their id only
            # borrows the blueprint's path, and their title is a pin name. Move
            # the id and leave the rest of the model alone.
            if "#" not in newId:
                # Repoint the model at the prim's new home. The Usd.Prim
                # captured at construction expired the moment the namespace edit
                # landed -- left in place it reads as "this node's prim is
                # gone", and the next resync would have
                # _validateNodesAgainstStage drop the node we just rescued.
                node._primPath = Sdf.Path(newId)
                if stage is not None:
                    node._prim = stage.GetPrimAtPath(newId)
                # The title is the prim's name, and the name changed.
                node.name = Sdf.Path(newId).name
            # Hit-testing resolves through the spatial index, and the link
            # rebuild that refills it is deferred to the next paint -- drop the
            # dead key now so a click landing first cannot hit it.
            try:
                self._spatialIndex.removeNode(oldId)
            except Exception:
                pass

        # Re-key the node dict in one pass, preserving insertion order: it is
        # the draw and iteration order, and a pop/re-insert would send every
        # renamed node to the back.
        self.nodeGraph.nodes = {
            remap.get(nodeId, nodeId): node for nodeId, node in self.nodes.items()
        }

        def _remapId(value):
            return remap.get(value, value)

        def _remapIdInTuple(value, index=0):
            if not value:
                return value
            asList = list(value)
            asList[index] = _remapId(asList[index])
            return tuple(asList)

        self._selectedNodes = {_remapId(n) for n in self._selectedNodes}
        self._marqueeBaseSelectedNodes = {
            _remapId(n) for n in self._marqueeBaseSelectedNodes
        }
        self._marqueePrevSelectedNodes = {
            _remapId(n) for n in self._marqueePrevSelectedNodes
        }
        self._reconciledPositionVersions = {
            _remapId(n): v for n, v in self._reconciledPositionVersions.items()
        }
        ...
        self._dragLinkSyntheticTarget = _remapIdInTuple(self._dragLinkSyntheticTarget)
        self._dragLinkResolvedTarget = _remapIdInTuple(self._dragLinkResolvedTarget)
        self._reconnectOldSourceNodeId = _remapId(self._reconnectOldSourceNodeId)
        self._reconnectOldTargetNodeId = _remapId(self._reconnectOldTargetNodeId)

        # The blueprint / container graphs scope themselves to the prim they were
        # opened on (see _graphScopeParentPath). If that prim moved, the scope
        # has to move with it or reconciliation starts measuring against a path
        # that no longer resolves and empties the graph.
        if self._currentPrimPath is not None:
            currentStr = str(self._currentPrimPath)
            for oldPath, newPath in movedPaths.items():
                if _isPathRelated(currentStr, oldPath) and len(currentStr) >= len(
                    oldPath
                ):
                    self._currentPrimPath = Sdf.Path(
                        newPath + currentStr[len(oldPath):]
                    )
                    break

        # Node ids are baked into the vertex, text and icon caches, so the old
        # id's geometry would linger while the new id drew nothing. Links are
        # re-derived from USD by the rebuild this schedules, which is also what
        # refills the spatial index.
        self._invalidateBackReferenceLinkDataCache()
        self._nodeRenderManager.invalidateAll()
        self._resetRenderCachesAfterBulkMove()

        Tf.Status(
            "Graph followed %d node(s) across a namespace move: %s"
            % (len(remap), ", ".join(f"{o} -> {n}" for o, n in sorted(remap.items())))
        )
        return remap

    def _handleUsdChanges(self, resyncedPaths, infoChangedPaths, movedPaths=None):
        ...
        Args:
            ...
            infoChangedPaths: Set of path strings that had property value changes
                             (positions, metadata)
            movedPaths: Optional ``{oldPath: newPath}`` for prims USD reports as
                        renamed or reparented rather than deleted and recreated.
        """
        ...
        # Follow moved prims FIRST. Both loops below, and the reconciliation
        # that follows them, ask "is this path one of my nodes?" -- so the
        # nodes have to be sitting under their new paths before any of that
        # runs, or the move reads as a delete of the old path plus an
        # unrelated create of the new one.
        movedNodes = self._remapMovedNodes(movedPaths) if movedPaths else {}
        ...
        # Reconcile the graph with the stage. The loops above only touch paths
        # that are already nodes, so neither of them can notice that a prim was
        # deleted, deactivated or renamed out from under one.
        if resyncedPaths:
            removed = self._validateNodesAgainstStage()
            adopted = self._adoptPrimsNewToScope(resyncedPaths)
    ...
    def initializeGL(self):
        ...
        plugin = Plug.Registry().GetPluginWithName("UsdNoodles")
    ...
    def _visibleWorldBounds(self):
        """What the camera currently shows, as a world-space rect.

        ``(panX, panY)`` is its top-left corner; the extent is the widget size
        divided by the zoom.
        """
        zoom = self.zoom if self.zoom else 1.0
        return Gf.Range2d(
            Gf.Vec2d(self.panX, self.panY),
            Gf.Vec2d(self.panX + self.width() / zoom, self.panY + self.height() / zoom),
        )

    def _isVisible(self, bounds):
        """Whether any part of *bounds* falls inside the current view."""
        if bounds.IsEmpty():
            return False
        return not Gf.Range2d.GetIntersection(
            self._visibleWorldBounds(), bounds
        ).IsEmpty()

    def _viewportAnchor(self):
        """Top-left of what the camera currently shows, in world space, inset.

        The inset is a fraction of the visible extent rather than a fixed
        distance so it reads as the same margin whatever the zoom.
        """
        visible = self._visibleWorldBounds()
        size = Gf.Vec2d(visible.GetSize())
        return (
            visible.GetMin()[0] + size[0] * 0.05,
            visible.GetMin()[1] + size[1] * 0.05,
        )

    def _batchExtents(self, placeable):
        """The (width, height) the grid below will actually occupy.

        Mirrors the layout loop exactly -- uniform columns sized to the widest
        node, rows only as tall as their own tallest node. Guessing high here
        is not "safe": an over-tall box collides with things the batch would
        never have reached, and each collision pushes the anchor further down,
        which is how a batch ends up nowhere near where it was asked to go.
        """
        cellW = max(n.size[0] for n in placeable) + self._AUTO_GRID_GAP
        cols = self._AUTO_GRID_COLUMNS
        width = cellW * min(len(placeable), cols)
        height = 0.0
        for rowStart in range(0, len(placeable), cols):
            rowNodes = placeable[rowStart : rowStart + cols]
            height += max(n.size[1] for n in rowNodes) + self._AUTO_GRID_GAP
        return width, height

    def _pushBelowOccupied(self, anchorX, anchorY, placeable, placed):
        """Slide ``anchorY`` down until the batch clears the nodes already placed.

        A caller-chosen anchor says *where the user is looking*, not *where
        there is room*; dropping a batch straight onto existing nodes would hide
        both. Each pass moves the anchor just under whatever it collides with
        and re-tests, bounded by the number of placed nodes because every pass
        clears at least one of them.
        """
        if not placed:
            return anchorY

        batchWidth, batchHeight = self._batchExtents(placeable)

        for _ in range(len(placed)):
            collided = False
            for node in placed:
                nx, ny = node.position[0], node.position[1]
                nw, nh = node.size[0], node.size[1]
                if (
                    anchorX < nx + nw
                    and anchorX + batchWidth > nx
                    and anchorY < ny + nh
                    and anchorY + batchHeight > ny
                ):
                    anchorY = ny + nh + self._AUTO_GRID_GAP
                    collided = True
                    break
            if not collided:
                break
    ...
    def _gridPlaceNodes(self, nodes, anchor=None):
        ...
        Args:
            nodes: Candidate nodes; only those still at (0, 0) are placed.
            anchor: Optional ``(x, y)`` in world space to start the batch at,
                for callers that need the result somewhere specific -- adding
                into the viewport the user is looking at, rather than below
                content that may be far off screen. The batch is still pushed
                clear of anything already placed that it would land on.
        """
        ...
        if anchor is not None:
            anchorX, anchorY = float(anchor[0]), float(anchor[1])
            anchorY = self._pushBelowOccupied(anchorX, anchorY, placeable, placed)
        elif placed:
    ...
    def _openSelectedContainer(self):
        ...
        from UsdNoodles import OpenNewNoodlesEditorForContainer
    ...
    def wheelEvent(self, event):
        ...
        if self.nodeCreationHotbox.is_visible:
            # Pixels first, degrees second.
            #
            # A macOS trackpad sends high-resolution pixelDelta, and the OS
            # keeps sending events after the fingers lift -- that IS the
            # momentum, so honouring the pixels gives native inertia for
            # free. The old `-angleDelta // 120` threw all of it away: a
            # trackpad's angleDelta arrives in single digits, integer-divided
            # by a 120-unit wheel notch to exactly 0, so nothing moved until
            # enough events had been discarded to cross a notch and the list
            # jumped a whole row. That is the choppiness.
            pixels = event.pixelDelta().y()
            if pixels:
                self.nodeCreationHotbox.handle_scroll_pixels(-float(pixels))
            else:
                # A real wheel (or any device without pixel resolution):
                # 120 units is one notch, three rows, as before.
                degrees = event.angleDelta().y()
                self.nodeCreationHotbox.handle_scroll_pixels(
                    -float(degrees) / 120.0 * 3.0
                    * self.nodeCreationHotbox.row_height())
            self.update()
            return
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        # A node under the cursor: rename it. Last, so the link and port cases
        # above keep the meaning they already had -- they hit-test geometry
        # that sits on top of a node, and stealing their double-click would
        # cost the only way to follow a dangling link.
        nodeId = self._getNodeIdAtWorldPos(worldPos)
        if nodeId:
            self._beginRename(nodeId)
            return
    ...
    def mousePressEvent(self, event):
        self.setFocus()
        # Clicking away commits, which is what a text field everywhere else
        # does. Committing before the click is dispatched matters: the click
        # may land on another node, and that node's selection should follow
    ...
    def mouseReleaseEvent(self, event):
        # Release the hotbox scrollbar first. It is the only overlay that
        # takes a press-drag-release, and the handlers below would otherwise
        # treat the release as ending a marquee or a link drag that never
        # started.
        if self.nodeCreationHotbox.is_visible:
            if self.nodeCreationHotbox.handle_mouse_release(
    ...
    def keyPressEvent(self, event):
        ...
        # A rename in progress owns the keyboard, and owns ALL of it: the
        # shortcuts below are single letters, so letting an unhandled key fall
        # through would make typing a name called Z undo the graph.
        if self._renamingNodeId:
            if event.key() in (QtCore.Qt.Key_Return, QtCore.Qt.Key_Enter):
                self._commitRename()
            elif event.key() == QtCore.Qt.Key_Escape:
                ...
                self._renameInput.handle_key(event)
            self.update()
            return
    ...
    def _reloadCurrentGraph(self):
        ...
        What the loader produces is not the whole graph, though. A loader walks
        the direct children of one scope; the canvas holds whatever the user put
        there, at any depth, via 'A' (``addNodesFromPrimTreeSelection``) or node
        creation under a prim-tree selection -- membership that lives only in
        ...
        # Membership and display-only placement: view state with no USD backing,
        # so the reload below is the only thing that could lose it.
        savedNodeIds = list(self.nodes.keys())
        savedDisplayPositions = {
            nid: Gf.Vec2d(node.position)
            for nid, node in self.nodes.items()
            if not self._nodeHasAuthoredPosition(node)
        }
        ...
        try:
            ...
            # Put back the nodes the user curated onto the canvas that the
            # loader's scope does not cover. A prim that genuinely went away
            # (deleted, or deactivated by an undo of its creation) is skipped --
            # the loaders apply the same rule.
            for nodeId in savedNodeIds:
                if nodeId in self.nodes or "#" in nodeId:
                    continue
                prim = stage.GetPrimAtPath(nodeId)
                if not prim or not prim.IsValid() or not prim.IsActive():
                    continue
                try:
                    self._addPrimAsNode(prim, stage, [])
                except Exception as error:
                    Tf.Warn("Graph reload could not re-add %s: %s" % (nodeId, error))

            # Re-apply placements that were never authored. A node whose
            # position IS authored is left alone -- USD owns that one, and the
            # reload may be undoing a move that authored it.
            for nodeId, position in savedDisplayPositions.items():
                node = self.nodes.get(nodeId)
                if node is not None and not self._nodeHasAuthoredPosition(node):
                    node.setDisplayPosition(position)
    ...
    def addNodesFromPrimTreeSelection(self):
        ...
        try:
            ...
            # Whether this add is populating an empty canvas or landing in a
            # graph the user has already arranged decides both where the batch
            # goes and whether the camera is allowed to move (see below).
            graphWasEmpty = not self.nodes
            ...
            if addedCount > 0 or selectedExistingCount > 0:
                ...
                # Move the camera only when the batch cannot be seen from where
                # it already is.
                #
                # Reframing on every add throws away the pan and zoom the user
                # chose, mid-task, every time they add a prim -- and it is not
                # needed, because the batch was just anchored into the viewport
                # above. But grid placement only moves nodes still at the USD
                # default (0, 0): a prim carrying an authored
                # ui:nodegraph:node:pos goes back to wherever it was last
                # dropped, which may be nowhere near the current view. Adding a
                # node the user cannot see and cannot find is worse than moving
                # the camera, so that case still frames.
                if addedNodeIds:
                    ...
                    if not addedBounds.IsEmpty() and (
                        graphWasEmpty or not self._isVisible(addedBounds)
                    ):
    ...
    def _createNodeFromHotbox(self, node_data, screen_pos):
        ...
        API schema entries are not node types: choosing one applies the schema
        to the selected prims instead of creating a prim.

        Args:
            node_data: Dict with 'identifier', 'library' and 'kind' keys from
                the hotbox
            screen_pos: Screen position tuple (x, y) where hotbox was opened
        """
        from .apiSchemas import API_SCHEMA_KIND

        if node_data.get("kind") == API_SCHEMA_KIND:
            self._applyApiSchemaFromHotbox(node_data["identifier"])
            return
    ...
    def _applyApiSchemaFromHotbox(self, identifier):
        """Apply an API schema chosen in the hotbox to the selected prims.

        Prims the schema forbids (its canOnlyApplyTo types) are skipped rather
        than authored, and reported in the summary popup. Multiple-apply
        schemas get a generated instance name per prim.

        Args:
            identifier: USD schema identifier, e.g. "PhysicsRigidBodyAPI"
        """
        from .apiSchemas import ALREADY_PRESENT, APPLIED, apply_to_prim

        selectedPrims = self._resolveSelectedPrims()
        if not selectedPrims:
            self._showWarningPopup(f"Select a prim to apply {identifier}")
            return

        applied = []  # (primPath, instanceName) pairs, for undo
        alreadyPresent = 0  # prims that already carried the schema
        skipped = []  # (primName, reason) pairs, for the summary

        self._noticeHandler.setEnabled(False)
        try:
            for prim in selectedPrims:
                status, instanceName, reason = apply_to_prim(prim, identifier)
                if status == APPLIED:
                    applied.append((str(prim.GetPath()), instanceName))
                elif status == ALREADY_PRESENT:
                    alreadyPresent += 1
                else:
                    skipped.append((prim.GetName(), reason))
        finally:
            self._noticeHandler.setEnabled(True)

        # Only the prims this invocation actually authored are undoable.
        # Reversing a no-op would strip a schema the prim already had.
        if applied:
            # Applying a schema changes which properties a prim has, so the
            # affected nodes need their pins rebuilt from USD.
            _push_undo_command(
                f"Apply {identifier}",
                _make_api_schema_undo(self, identifier, applied, apply=True),
                _make_api_schema_undo(self, identifier, applied, apply=False),
            )
            self._reloadCurrentGraph()

        self._showApiSchemaResult(identifier, applied, alreadyPresent, skipped)

    def _showApiSchemaResult(self, identifier, applied, alreadyPresent, skipped):
        """Report what an API schema application did.

        Args:
            identifier: Schema identifier that was applied
            applied: List of (primPath, instanceName) pairs newly authored
            alreadyPresent: Count of prims that already carried the schema
            skipped: List of (primName, reason) pairs that were skipped
        """
        if not applied and not alreadyPresent:
            reason = skipped[0][1] if skipped else "no prims selected"
            self._showWarningPopup(f"Could not apply {identifier}: {reason}")
            return

        if applied:
            noun = "prim" if len(applied) == 1 else "prims"
            summary = f"Applied {identifier} to {len(applied)} {noun}"
        else:
            noun = "prim" if alreadyPresent == 1 else "prims"
            summary = f"{identifier} already on {alreadyPresent} {noun}"

        notes = []
        if applied and alreadyPresent:
            notes.append(f"{alreadyPresent} already had it")
        if skipped:
            names = ", ".join(name for name, _ in skipped[:3])
            if len(skipped) > 3:
                names += f", +{len(skipped) - 3} more"
            notes.append(f"skipped {len(skipped)}: {names}")

        if not notes:
            self._showPopupMessage(summary)
            return

        message = f"{summary} ({'; '.join(notes)})"
        if skipped:
            self._showWarningPopup(message)
        else:
            self._showPopupMessage(message)

    def _resolveSelectedPrims(self):
        """Return the prims the user means by "the selection" in usdview.

        Clicking the Noodles canvas resets usdview's selection to the pseudo-
        root, so the live selection alone is not trustworthy while the editor
        has focus. Fall back to the last meaningful prim tree selection, the
        same way addNodesFromPrimTreeSelection does. The pseudo-root and
        duplicates (getPrims can repeat a prim via instance proxies) are
        dropped.

        Returns:
            List[Usd.Prim]: Selected prims, empty if nothing is selected.
        """
        if not self._usdviewApi:
            return []

        dataModel = getattr(self._usdviewApi, "dataModel", None)
        if not dataModel or not dataModel.selection:
            return []

        liveNonRoot = [
            prim
            for prim in dataModel.selection.getPrims()
            if prim and prim.IsValid() and prim.GetPath().pathString != "/"
        ]
        cachedNonRoot = [
            prim
            for prim in self._lastPrimTreeSelection
            if prim and prim.IsValid() and prim.GetPath().pathString != "/"
        ]
        if len(cachedNonRoot) > len(liveNonRoot):
            selectedPrims = cachedNonRoot
        else:
            selectedPrims = liveNonRoot or cachedNonRoot

        seenPaths = set()
        uniquePrims = []
        for prim in selectedPrims:
            pathStr = str(prim.GetPath())
            if pathStr in seenPaths:
                continue
            seenPaths.add(pathStr)
            uniquePrims.append(prim)
        return uniquePrims

    def _resolveSelectedParentPrim(self):
        """Return the prim new prims should be created under, or None.

        Prefers usdview's focus prim - the last one clicked, which its own UI
        treats as primary - and falls back to the last prim of the resolved
        selection when the focus prim is unusable, which is what usdview
        reports after a click on the Noodles canvas.
        """
        selectedPrims = self._resolveSelectedPrims()
        if not selectedPrims:
            return None
        ...
        dataModel = getattr(self._usdviewApi, "dataModel", None)
        selection = getattr(dataModel, "selection", None) if dataModel else None
        getFocusPrim = getattr(selection, "getFocusPrim", None)
        if getFocusPrim is not None:
            try:
                focusPrim = getFocusPrim()
            except Exception:
                focusPrim = None
            if focusPrim and focusPrim.IsValid():
                focusPath = str(focusPrim.GetPath())
                if any(str(prim.GetPath()) == focusPath for prim in selectedPrims):
                    return focusPrim

        return selectedPrims[-1]

    def _resolveParentPath(self, stage):
        """Resolve the parent path for node creation.

        The usdview selection wins: new prims are created under the prim
        focused in the prim tree. With nothing selected, fall back to the
        container the graph is currently displaying, then to the first
        Blueprint on the stage, then to the root.
        """
        from .primAuthoring import PrimAuthor

        selectedParent = self._resolveSelectedParentPrim()
        if selectedParent is not None:
            return selectedParent.GetPath()
    ...
    def _beginRename(self, nodeId):
        ...
        # Blueprint I/O nodes are synthesized rather than backed by a prim, so
        # there is no name to change.
        if "#" in nodeId:
            self._showPopupMessage("This node is not backed by a prim")
            return

        stage = self._usdviewApi.stage if self._usdviewApi else None
        prim = stage.GetPrimAtPath(nodeId) if stage else None
        if not prim or not prim.IsValid():
            self._showWarningPopup("Cannot rename: the prim is gone")
            return

        self._renamingNodeId = nodeId
        self._renameInput.text = prim.GetName()
        self._renameInput.cursor_position = len(self._renameInput.text)
        self.update()

    def _cancelRename(self):
        """Abandon the inline editor without authoring anything."""
        self._renamingNodeId = ""
        self._renameInput.clear()
        self.update()

    def _commitRename(self):
        """Apply the typed name to the prim, with everything a rename implies.

        A recoverable objection -- an invalid identifier, a name a sibling
        already has -- leaves the editor open so the typo can be fixed. A
        refusal from USD closes it, because no amount of retyping changes a
        composition arc.
        """
        from .primRename import (
            INVALID_NAME,
            NAME_TAKEN,
            RENAMED,
            UNCHANGED,
        ...
        nodeId = self._renamingNodeId
        newName = self._renameInput.text.strip()
        if not nodeId:
            return
        ...
        oldName = prim.GetName()
        status, newPath, message = rename_prim(stage, prim, newName)

        if status == UNCHANGED:
            self._cancelRename()
            return

        if status in (INVALID_NAME, NAME_TAKEN):
            self._showWarningPopup(message)
            return  # stay open; the name is fixable

        if status != RENAMED:
            self._cancelRename()
            self._showWarningPopup(f"Cannot rename {oldName}: {message}")
            return

        self._renamingNodeId = ""
        self._renameInput.clear()

        # The resync notice the rename emitted has already moved the node to its
        # new path (USD classifies the pair as RenameSource / RenameDestination,
        # and _handleUsdChanges follows it). This call is the belt-and-braces
        # half for the cases where it did not -- notices suppressed by an
        # enclosing edit, or a USD build without GetPrimResyncType -- and is a
        # no-op when the node is already where it belongs.
        #
        # What must NOT happen here is a reload. Graph membership is the user's
        # ('A' adds prims at any depth, 'D' takes them off, neither touches USD)
        # and no loader can reproduce it, so re-running one would answer a
        # rename by emptying the canvas down to whatever the loader's scope
        # holds.
        newNodeId = str(newPath)
        self._remapMovedNodes({nodeId: newNodeId})
        ...
        _push_undo_command(
            ...
            _make_rename_undo(gv, str(newPath.GetParentPath()), oldName, newName),
            _make_rename_undo(gv, str(newPath.GetParentPath()), newName, oldName),
        )
    ...
    def _paintNodeRename(self):
        ...
        node = self.nodes.get(self._renamingNodeId)
        if node is None:
            return

        projection = self._screenSpaceProjectionMatrix()
        x = (float(node.position[0]) - self.panX) * self.zoom
        y = (float(node.position[1]) - self.panY) * self.zoom
        # Never narrower than the text: a zoomed-out node is a few pixels wide
        # and the field has to stay legible to be typed into.
        minWidth, _ = self._renameInput.calculate_size(self.textRenderer)
        width = max(float(node.size[0]) * self.zoom, minWidth)

        self._renameInput.render(
            self.textRenderer,
            x,
            y,
            MAX_RENDER_DEPTH,
    ...
    def _onNodeCreated(self, prim, prim_path, stage, identifier):
        ...
        if newNodeId in self.nodes:
            self.nodes[newNodeId].selected = True
            self._selectedNodes.add(newNodeId)
            # ...and carry that into usdview, which is the other half of
            # "the new prim is selected". Marking node.selected only lights
            # it up on this canvas; the prim tree, the property editor and
            # the viewport all read dataModel.selection, and nothing else on
            # the creation path writes it.
            #
            # Every other selecting path already syncs -- click, marquee,
            # Select All, navigate-to-connected. Creation was the one that
            # did not, so a freshly made prim was the only way to end up
            # with the two halves of the app disagreeing about what is
            # selected.
            self._syncSelectionToPrimtree()
