...
try:
    ...
    from UsdNoodles.models import _cpp_position, NodeModel
...
class TestSetDisplayPositionNoUsdAuthor(unittest.TestCase):
    ...
    def test_updates_cpp_side_node_data_position(self):
        ...
        by ``UsdNoodles.models`` (the same ``NodeData.__dict__['position']``
...
def _anchored_grid_view(nodes_by_id):
    """A ``_grid_view`` that can also run the caller-anchored branch."""
    view = _grid_view(nodes_by_id)
    view._batchExtents = lambda placeable: GraphView._batchExtents(view, placeable)
    view._pushBelowOccupied = lambda ax, ay, placeable, placed: (
        GraphView._pushBelowOccupied(view, ax, ay, placeable, placed)
    )
    return view


@unittest.skipUnless(_has_pxr, "UsdNoodles not available")
class TestGridPlaceAtACallerAnchor(unittest.TestCase):
    """``_gridPlaceNodes(anchor=...)``: place the batch where asked.

    The default anchor puts a batch below everything already placed, which is
    tidy but can be an entire screen away from what the camera is showing. The
    add-from-prim-tree path needs the opposite guarantee -- land in view,
    because the camera is deliberately not going to move to meet the batch.
    """

    def test_the_anchor_is_used_instead_of_the_world_origin(self):
        node = _grid_node("/N")
        view = _anchored_grid_view({"/N": node})

        GraphView._gridPlaceNodes(view, [node], anchor=(700.0, 400.0))

        self.assertEqual(node.position, (700.0, 400.0))

    def test_the_anchor_is_used_instead_of_below_existing_content(self):
        """Without an anchor this batch would go below /Far, thousands of
        units away from where the user is looking."""
        far = _grid_node("/Far", position=(0.0, 9000.0))
        node = _grid_node("/N")
        view = _anchored_grid_view({"/Far": far, "/N": node})

        GraphView._gridPlaceNodes(view, [node], anchor=(700.0, 400.0))

        self.assertEqual(node.position, (700.0, 400.0))

    def test_no_anchor_still_goes_below_existing_content(self):
        """The default is unchanged -- the anchor is opt-in."""
        existing = _grid_node("/Existing", position=(10.0, 200.0))
        node = _grid_node("/N")
        view = _anchored_grid_view({"/Existing": existing, "/N": node})

        GraphView._gridPlaceNodes(view, [node])

        self.assertEqual(node.position[0], 10.0)
        self.assertEqual(node.position[1], 200.0 + 50.0 + GraphView._AUTO_GRID_GAP)

    def test_the_batch_is_pushed_clear_of_what_it_would_land_on(self):
        """An anchor says where the user is looking, not where there is room."""
        occupant = _grid_node("/Occupant", position=(700.0, 400.0))
        node = _grid_node("/N")
        view = _anchored_grid_view({"/Occupant": occupant, "/N": node})

        GraphView._gridPlaceNodes(view, [node], anchor=(700.0, 400.0))

        self.assertEqual(node.position[0], 700.0)
        self.assertEqual(node.position[1], 400.0 + 50.0 + GraphView._AUTO_GRID_GAP)

    def test_a_clear_anchor_is_left_alone(self):
        """Nodes elsewhere must not push the batch around for no reason."""
        elsewhere = _grid_node("/Elsewhere", position=(-5000.0, -5000.0))
        node = _grid_node("/N")
        view = _anchored_grid_view({"/Elsewhere": elsewhere, "/N": node})

        GraphView._gridPlaceNodes(view, [node], anchor=(700.0, 400.0))

        self.assertEqual(node.position, (700.0, 400.0))

    def test_a_wrapping_batch_is_measured_by_its_rows_not_its_node_count(self):
        """The collision box must be the height the grid will ACTUALLY occupy.

        Summing every node's height treats a wrapping batch as one tall column
        -- a 60-node batch measures 30x too tall, collides with things it would
        never have reached, and each collision pushes the anchor further down
        until the batch lands nowhere near the viewport it was aimed at.
        """
        cols = GraphView._AUTO_GRID_COLUMNS
        gap = GraphView._AUTO_GRID_GAP
        # Two full rows of 50-tall nodes: the batch occupies 2 * (50 + gap).
        batch = [_grid_node(f"/N{i}") for i in range(cols * 2)]
        # An occupant just below that true height -- inside the over-estimated
        # box, outside the real one.
        clear_y = 400.0 + 2 * (50.0 + gap) + 10.0
        occupant = _grid_node("/Occupant", position=(700.0, clear_y))
        nodes = {n.id: n for n in batch}
        nodes["/Occupant"] = occupant
        view = _anchored_grid_view(nodes)

        GraphView._gridPlaceNodes(view, batch, anchor=(700.0, 400.0))

        self.assertEqual(batch[0].position, (700.0, 400.0))

    def test_the_push_terminates_on_a_wall_of_nodes(self):
        """Every pass clears at least one occupant, so the loop is bounded by
        their count -- a stacked column must not spin."""
        occupants = {}
        for i in range(40):
            occupant = _grid_node(f"/Occ{i}", position=(700.0, 400.0 + i * 200.0))
            occupants[occupant.id] = occupant
        node = _grid_node("/N")
        occupants["/N"] = node
        view = _anchored_grid_view(occupants)

        GraphView._gridPlaceNodes(view, [node], anchor=(700.0, 400.0))

        lowest = max(o.position[1] + o.size[1] for o in occupants.values() if o is not node)
        self.assertGreaterEqual(node.position[1], lowest)


@unittest.skipUnless(_has_pxr, "UsdNoodles not available")
class TestViewportAnchor(unittest.TestCase):
    """``_viewportAnchor`` reports the corner of what the camera shows."""

    def _view(self, panX, panY, zoom):
        view = SimpleNamespace(
            panX=panX,
            panY=panY,
            zoom=zoom,
        ...
        view._visibleWorldBounds = lambda: GraphView._visibleWorldBounds(view)
        return view

    def test_it_is_the_top_left_inset_by_a_fraction_of_the_view(self):
        anchor = GraphView._viewportAnchor(self._view(1000.0, 2000.0, 1.0))

        self.assertEqual(anchor, (1000.0 + 40.0, 2000.0 + 30.0))

    def test_the_inset_scales_with_zoom_so_it_looks_the_same(self):
        """At half zoom twice as much world fits on screen, so the same
        on-screen margin is twice the world distance."""
        anchor = GraphView._viewportAnchor(self._view(0.0, 0.0, 0.5))

        self.assertEqual(anchor, (80.0, 60.0))

    def test_a_zero_zoom_does_not_divide_by_zero(self):
        anchor = GraphView._viewportAnchor(self._view(5.0, 6.0, 0.0))

        self.assertEqual(anchor, (5.0 + 40.0, 6.0 + 30.0))


@unittest.skipUnless(_has_pxr, "UsdNoodles not available")
class TestIsVisible(unittest.TestCase):
    """``_isVisible``: is this rect anywhere on screen right now.

    It decides whether an add is allowed to move the camera. Grid-placed nodes
    land in view and must answer True (camera stays put); a node parked at an
    authored position far away answers False (frame it, or the user is told
    something was added and can see nothing).
    """

    def _view(self, panX=0.0, panY=0.0, zoom=1.0):
        view = SimpleNamespace(
            panX=panX,
            panY=panY,
            zoom=zoom,
    ...
    def _rect(self, x, y, w=100.0, h=50.0):
        return Gf.Range2d(Gf.Vec2d(x, y), Gf.Vec2d(x + w, y + h))

    def test_a_rect_in_the_middle_of_the_view_is_visible(self):
        self.assertTrue(GraphView._isVisible(self._view(), self._rect(300.0, 200.0)))

    def test_a_rect_far_away_is_not_visible(self):
        self.assertFalse(
            GraphView._isVisible(self._view(), self._rect(40000.0, 40000.0))
        )

    def test_a_rect_straddling_the_edge_is_visible(self):
        self.assertTrue(GraphView._isVisible(self._view(), self._rect(-50.0, 200.0)))

    def test_the_pan_is_honored(self):
        rect = self._rect(5000.0, 5000.0)
        self.assertFalse(GraphView._isVisible(self._view(), rect))
        self.assertTrue(
            GraphView._isVisible(self._view(panX=4900.0, panY=4900.0), rect)
        )

    def test_zooming_out_brings_more_into_view(self):
        rect = self._rect(3000.0, 2000.0)
        self.assertFalse(GraphView._isVisible(self._view(zoom=1.0), rect))
        self.assertTrue(GraphView._isVisible(self._view(zoom=0.1), rect))

    def test_an_empty_rect_is_not_visible(self):
        self.assertFalse(GraphView._isVisible(self._view(), Gf.Range2d()))
