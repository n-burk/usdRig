...
try:
    from UsdNoodles.nodeGraph import NodeGraph

    _has_pxr = True
except ImportError as e:
    print(f"UsdNoodles.nodeGraph import failed: {e}")
    _has_pxr = False

try:
    from UsdNoodles.core import RenderConfig
...
try:
    from UsdNoodles.nodeGraph import build_render_config, createNodeRenderer

    _has_helpers = True
except ImportError as e:
    print(f"UsdNoodles.nodeGraph config helpers import failed: {e}")
    _has_helpers = False
...
def _patch_config(values):
    ...
    Installs a fake ``UsdNoodles.noodlesConfig`` module in ``sys.modules``
    ...
    target — ``mock.patch("UsdNoodles.noodlesConfig.NoodlesConfig.get")``
    would error at context-manager entry.
    """
    fake = types.ModuleType("UsdNoodles.noodlesConfig")
    ...
    return mock.patch.dict(sys.modules, {"UsdNoodles.noodlesConfig": fake})
...
@unittest.skipUnless(_has_config, "UsdNoodles config helpers not available")
...
class TestCreateNodeRenderer(unittest.TestCase):
    ...
    def test_passes_render_config_to_renderer(self):
        ...
        with (
            mock.patch(
                "UsdNoodles.nodeGraph._get_renderer_registry",
