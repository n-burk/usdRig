...
@unittest.skipUnless(_has_pxr, "UsdNoodles not available")
class TestInvalidateCacheKeepsDisplayOnlyPositions(unittest.TestCase):
    """``NodeModel.invalidateCache`` re-reads USD. A display-only position is
    not in USD.

    Auto-placement (``_gridPlaceNodes``, auto-layout previews) goes through
    ``setDisplayPosition``, which deliberately never authors
    ``ui:nodegraph:node:pos`` -- so opening a stage cannot dirty it and a prim
    that refuses the attribute cannot break placement. The consequence is that
    there is nothing to re-read: dropping the cached value sends the node to
    the position getter's unauthored fallback of (0, 0). Since a resync
    invalidates every node it touches, that turned any edit near a hand-placed
    graph into the whole graph collapsing onto the origin.
    """
    ...
    def setUp(self):
        ...
        self.stage.DefinePrim("/Node", "Xform")
        self.node = NodeModel(stage=self.stage, primPath="/Node")

    def test_a_display_only_position_survives_invalidation(self):
        self.node.setDisplayPosition(self.Gf.Vec2d(500.0, 300.0))

        self.node.invalidateCache()

        self.assertEqual(tuple(self.node.position), (500.0, 300.0))

    def test_an_authored_position_is_dropped_and_re_read(self):
        """USD owns that one, so invalidation must still pick up a change."""
        from pxr import Sdf

        attr = self.stage.GetPrimAtPath("/Node").CreateAttribute(
            "ui:nodegraph:node:pos", Sdf.ValueTypeNames.Float2
        )
        attr.Set(self.Gf.Vec2f(1.0, 1.0))
        self.assertEqual(tuple(self.node.position), (1000.0, 1000.0))

        attr.Set(self.Gf.Vec2f(2.0, 3.0))
        self.node.invalidateCache()

        self.assertEqual(tuple(self.node.position), (2000.0, 3000.0))

    def test_everything_else_is_still_invalidated(self):
        """Keeping the position must not turn invalidateCache into a no-op."""
        self.assertEqual(self.node.name, "Node")
        self.node.setDisplayPosition(self.Gf.Vec2d(500.0, 300.0))

        self.node.invalidateCache()

        self.assertIsNone(self.node._name)
        self.assertIsNone(self.node._type)
        self.assertIsNone(self.node._input_pins)

    def test_a_node_with_no_position_yet_is_unaffected(self):
        self.node.invalidateCache()

        self.assertIsNone(self.node._position)

    def test_has_authored_position_reports_what_usd_holds(self):
        from pxr import Sdf

        self.assertFalse(self.node.hasAuthoredPosition())

        self.stage.GetPrimAtPath("/Node").CreateAttribute(
            "ui:nodegraph:node:pos", Sdf.ValueTypeNames.Float2
        ).Set(self.Gf.Vec2f(1.0, 1.0))

        self.assertTrue(self.node.hasAuthoredPosition())

    def test_an_expired_prim_has_no_authored_position(self):
        self.stage.RemovePrim("/Node")
