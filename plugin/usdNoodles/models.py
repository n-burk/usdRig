...
from UsdNoodles.core import NodeData
...
class NodeModel(NodeData):
    ...
    def hasAuthoredPosition(self):
        """Whether USD holds an opinion for this node's ``ui:nodegraph:node:pos``.

        False means the cached position is view state alone (a grid placement,
        an auto-layout preview) with nothing on the stage to restore it from.
        """
        if not self._prim or not self._prim.IsValid():
            return False
        posAttr = self._prim.GetAttribute("ui:nodegraph:node:pos")
    ...
    def invalidateCache(self):
        ...
        A *display-only* position survives, because there is nothing in USD to
        re-read it from: ``_gridPlaceNodes`` and friends place nodes through
        ``setDisplayPosition``, which deliberately never authors
        ``ui:nodegraph:node:pos``. Dropping it here would send the node back to
        the position getter's unauthored fallback of (0, 0) -- so any resync
        that merely touched the node (a rename, a neighbour's connection
        changing) would collapse the whole hand-placed graph onto the origin.
        An authored position is still dropped: USD owns that one.
        """
        keptPosition = None
        if self._position is not None and not self.hasAuthoredPosition():
            keptPosition = self._position
        self._name = None
        self._type = None
        self._schemaTypeName = None
        self._position = keptPosition
        self._input_pins = None
        self._output_pins = None
        self._input_pin_types = None
        self._output_pin_types = None
