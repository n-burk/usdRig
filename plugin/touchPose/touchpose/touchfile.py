...
maps a the conventional tool `objectSet` name to its record. One entry is special:
  ...
  * every other entry is a SET:
      - `members`: face components, each `"<mesh>.f[i]"` or
        ...
        trailing `"<setname>_data"` which is a the conventional tool node, not geometry,
        and is dropped here.
      ...
      - `touchColor`: the set's own linear RGB, which is what the the conventional tool
      ...
      - `command` / `touch_layers`: the conventional tool right-click menus, verbatim
        ...
        a the conventional tool dependency into the bottom layer for nothing.
...
been verified against the the conventional tool writer:
...
class TouchDocument(object):
    ...
    def __init__(self, payload):
        ...
        # the draw order the the conventional tool product used for overlapping regions.
