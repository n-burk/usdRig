...
_ModeName(rigExec::RigExecEvaluationMode mode)
{
    ...
    case rigExec::RigExecEvaluationMode::Sparse: return "sparse";
    case rigExec::RigExecEvaluationMode::SparseWithParityCheck:
        return "sparseParity";
...
_ParseMode(const std::string &name)
{
    ...
    if (name == "sparse") return rigExec::RigExecEvaluationMode::Sparse;
    if (name == "sparseParity") {
        return rigExec::RigExecEvaluationMode::SparseWithParityCheck;
    }
    if (name != "dynamic") {
        throw py::value_error(
            "evaluation_mode must be 'dynamic', 'baked', 'parity', 'sparse' "
            "or 'sparseParity' (got '" + name + "')");
    }
...
PYBIND11_MODULE(_rigexec, m) {
    ...
    py::class_<rigExec::RigExecRigPose>(m, "Pose",
        ...
        .def_property_readonly("sparse_parity_mismatches",
            [](const rigExec::RigExecRigPose &p) {
                return p.sparseParityMismatches;
            })
        .def_readonly("sparse_constraints_replayed",
            &rigExec::RigExecRigPose::sparseConstraintsReplayed)
        .def_readonly("sparse_constraints_solved",
            &rigExec::RigExecRigPose::sparseConstraintsSolved)
        .def_readonly("sparse_solver_batches_reused",
            &rigExec::RigExecRigPose::sparseSolverBatchesReused)
