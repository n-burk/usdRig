...
A 24-quad strip stands seven units tall over an invisible
four-CV B-spline driver, and the driver's control points swing on a
staggered cycle: the strip curls up one side until its tip has travelled
2.5 units across and dropped 1.7, sweeps back through upright around
frame 1012, then whips 3.8 units out the other way and recovers, closing
the loop exactly at frame 1024. `rigExec:sampleCount` is nine, a curve
mover in `ribbon` mode rides the strip along the run through its
`primvars:st` bind coordinates, and a nine-joint chain named on
`rigExec:joints` takes the same nine samples so the run reads as a dense
spine inside the bending ribbon.
