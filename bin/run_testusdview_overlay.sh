#!/bin/bash
# bin/run_testusdview_overlay.sh -- headless check that the volumetric influence overlay reaches a real viewport
# The POSIX twin of run_testusdview_overlay.bat.
#
# Usage: bin/run_testusdview_overlay.sh [rendererDisplayName]   (e.g. Embree)
set -euo pipefail
...
STAGE="$RIG/examples/11_VolumeWeights.usda"
...
exec "$PY" "$TESTUSDVIEW" --testScript "$RIG/tests/testUsdviewVolumeWeightOverlay.py" \
     "$@" "$STAGE"
