@echo off
rem Headless end-to-end test of the Control Picker panel
rem (tests\testUsdviewPicker.py) on examples\biped\Biped_stack.usda. The Windows twin
...
rem list rather than emptying it. Prints RIGEXEC_PICKER_OK.
rem
rem Usage: run_testusdview_execstack.bat [rendererDisplayName]  (e.g. Embree)
rem Set RIGEXEC_PICKER_SHOT=path.png to keep a window grab.
call "%~dp0_vcvars.bat"
call "%~dp0_env.bat"
...
"%PY%" "%USD%\bin\testusdview" --testScript "%RIG%\tests\testUsdviewPicker.py" %RENDERER_ARG% "%RIG%\examples\biped\Biped_stack.usda"
exit /b %errorlevel%
