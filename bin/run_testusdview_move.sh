#!/bin/bash
# bin/run_testusdview_move.sh -- curvenet move verification against the puppetA
# character. The POSIX twin of run_testusdview_move.bat.
#
# Usage: bin/run_testusdview_move.sh [stage.usda]
...
exec "$PY" "$TESTUSDVIEW" --testScript "$RIG/tests/testUsdviewCurvenetMove.py" "$STAGE"
