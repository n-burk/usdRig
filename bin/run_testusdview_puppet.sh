#!/bin/bash
# bin/run_testusdview_puppet.sh -- curvenet puppet verification against the puppetA
# character. The POSIX twin of run_testusdview_puppet.bat.
#
# Usage: bin/run_testusdview_puppet.sh [stage.usda]
...
exec "$PY" "$TESTUSDVIEW" --testScript "$RIG/tests/testUsdviewCurvenetPuppet.py" "$STAGE"
